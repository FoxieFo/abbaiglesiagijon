import { Church, Facebook, Instagram, Youtube, Heart } from "lucide-react";
import Whatsapp from "./../images/icons/whatsapp.svg";
import Logo from "./../images/logo/logo-light.svg";

const Footer = () => {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-church-dark-purple text-white pt-16 pb-8">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          <div>
            <img src={Logo} alt="logo" />
          </div>

          <div>
            <h3 className="text-lg font-semibold mb-4 text-white">Horarios</h3>
            <ul className="space-y-3">
              <li className="text-white">
                <span className="block font-medium">Martes</span>
                <span className="text-sm">Oración - 20:00</span>
              </li>
              <li className="text-white">
                <span className="block font-medium">Viernes</span>
                <span className="text-sm">Discipulado - 20:00</span>
              </li>
              <li className="text-white">
                <span className="block font-medium">Domingo</span>
                <span className="text-sm">Reunión Familiar - 11:00</span>
              </li>
            </ul>
          </div>
          <div>
            <h3 className="text-lg font-semibold mb-4 text-white">Contactos</h3>
            <a
              href="mailto:abbaiglesiagijon@gmail.com"
              className="text-white hover:text-church-gold transition mb-2 block "
            >
              abbaiglesiagijon@gmail.com
            </a>
            <a
              href="tel:+34675042136"
              className="text-white hover:text-church-gold transition block"
            >
              +34 675 042 136
            </a>
          </div>
          <div>
            <h3 className="text-lg font-semibold mb-4 text-white">
              Enlaces Rápidos
            </h3>
            <div className="flex flex-col space-x-4">
              <a
                href="https://wa.me/34675042136"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-white hover:text-church-gold transition-colors mb-4"
              >
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
                >
                  <path
                    d="M17 2.91C15.13 1.03 12.64 0 9.99 0C4.53 0 0.08 4.45 0.08 9.91C0.08 11.66 0.54 13.36 1.4 14.86L0 20L5.25 18.62C6.7 19.41 8.33 19.83 9.99 19.83C15.45 19.83 19.9 15.38 19.9 9.92C19.9 7.27 18.87 4.78 17 2.91ZM9.99 18.15C8.51 18.15 7.06 17.75 5.79 17L5.49 16.82L2.37 17.64L3.2 14.6L3 14.29C2.18 12.98 1.74 11.46 1.74 9.91C1.74 5.37 5.44 1.67 9.98 1.67C12.18 1.67 14.25 2.53 15.8 4.09C17.36 5.65 18.21 7.72 18.21 9.92C18.23 14.46 14.53 18.15 9.99 18.15ZM14.51 11.99C14.26 11.87 13.04 11.27 12.82 11.18C12.59 11.1 12.43 11.06 12.26 11.3C12.09 11.55 11.62 12.11 11.48 12.27C11.34 12.44 11.19 12.46 10.94 12.33C10.69 12.21 9.89 11.94 8.95 11.1C8.21 10.44 7.72 9.63 7.57 9.38C7.43 9.13 7.55 9 7.68 8.87C7.79 8.76 7.93 8.58 8.05 8.44C8.17 8.3 8.22 8.19 8.3 8.03C8.38 7.86 8.34 7.72 8.28 7.6C8.22 7.48 7.72 6.26 7.52 5.76C7.32 5.28 7.11 5.34 6.96 5.33C6.81 5.33 6.65 5.33 6.48 5.33C6.31 5.33 6.05 5.39 5.82 5.64C5.6 5.89 4.96 6.49 4.96 7.71C4.96 8.93 5.85 10.11 5.97 10.27C6.09 10.44 7.72 12.94 10.2 14.01C10.79 14.27 11.25 14.42 11.61 14.53C12.2 14.72 12.74 14.69 13.17 14.63C13.65 14.56 14.64 14.03 14.84 13.45C15.05 12.87 15.05 12.38 14.98 12.27C14.91 12.16 14.76 12.11 14.51 11.99Z"
                    fill="currentColor"
                  />
                </svg>
                Escríbenos
              </a>

              <a
                href="https://www.instagram.com/abbaiglesiagijon"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 hover:text-church-gold text-white transition-colors"
              >
                <Instagram size={20} />
                Síguenos
              </a>
            </div>
          </div>
        </div>

        <div className="border-t border-church-purple pt-8 mt-8 text-center">
          <p className="text-church-purple text-sm flex items-center justify-center">
            &copy; {currentYear} ABBA Iglesia Evangélica. Todos los derechos
            reservados.
          </p>
          <p className="text-church-purple text-xs mt-2 flex items-center justify-center gap-1 text-center">
            Creado con <Heart className="h-3 w-3 text-church-gold" /> para la
            gloria de Dios por{" "}
            <a
              href="https://www.linkedin.com/in/victoriaanisimova/"
              target="_blank"
              rel="noopener noreferrer"
              className="text-amber-50 hover:text-church-gold transition"
            >
              Victoria A.
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
